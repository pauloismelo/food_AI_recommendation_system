...
export class ModelView extends View {
    ...
    updateTrainingProgress(progress) {
        ...
        if (progress.progress === 100) {
            ...
            this.#trainModelBtn.innerHTML = '<i class="bi bi-lightning-charge-fill"></i> Train Model';
        }
    }
        ...
        const html = users.map(user => {
            ...
            return `
                <div class="user-purchase-summary">
                    <h6>${user.name} (Age: ${user.age}) &middot; <small>${user.region ? user.region.replace(/_/g, ' ') : 'N/A'}</small></h6>
                    <div class="purchases-badges">
                        ${purchasesHtml || '<span class="text-muted">No orders</span>'}
                    </div>
                </div>
            `;
        }).join('');
